---
id: "allzy.framework.prompts.specification.yin-yang-prompt-generator.universal"
title: "Yin/Yang Prompt Generator — Universal"
...
Use this prompt to generate a specialized Agent-2 prompt for creating Allzy Framework Yin/Yang specification documents.

This prompt does not directly create the final Yin/Yang document.

This prompt creates a precise prompt for another AI agent.

Agent 1 generates the prompt.

Agent 2 uses the generated prompt and the required attached template to create the final specification document.

---

## When to Use This Prompt

Use this prompt when you need to generate a task-specific prompt for creating one of these artifacts:
...
Use this prompt after Genesis, Segmentation, and Modularization have produced enough source context, such as:
...
- submodule block
- Splice block
- implementation topic block
- relevant source notes
- existing Yin/Yang references

Use this prompt when you want Agent 2 to create the final document using the correct Allzy template.
...
If the user wants direct document generation instead of prompt generation, explain that this generator produces an Agent-2 prompt. Agent 2 must then use the required template to create the final document.
...
If the user requests Yang Compact, respond:
...
The uploaded `04_yin_yang_template.md` confirms the template boundary: the Yin section follows the configured Yin output language; the Yang section is English by default in the current template; technical identifiers remain English; and if full Yang is not needed, the compact Yin/Yang template should be used instead. It also states that secrets must never be included. 
...
- mark missing technical details as Contract Gaps, or
...
If conversion is enabled, use these config fields:
...
Conversion mode changes the Agent-2 source instructions.

It does not change the target artifact rules.

The target must still be one of:
...
Your job is to generate the Agent-2 prompt.

Agent 2 writes the final specification document.

If the required template is missing, the Agent-2 prompt must stop and ask for it.
